...
# [ PRO ] yfinance SSL Bypass (v3.0 - Let YF handle anti-bot)
ssl._create_default_https_context = ssl._create_unverified_context
requests.packages.urllib3.disable_warnings(category=InsecureRequestWarning)
...
# --- [ CONFIG ] NOTICE_SHEET_URL DEPRECATED ---
...
if not st.session_state["password_correct"]:
    ...
    with m:
        ...
        # [ NOTICE ] 기존 고정 공지 섹션 삭제 및 이미지 강화
        if os.path.exists("DragonFly.png"):
            st.image("DragonFly.png", use_container_width=True)
        elif logo_b64:
            st.markdown(
            ...
            if st.button("Terminal Operation Start", use_container_width=True):
                ...
                if login_id in users:
                    ...
                    if u_data.get("status", "approved") != "approved":
                        ...
                        st.session_state.login_time = time.time() # 접속 시각 기록
                        st.rerun()
                    else:
        ...
        with tab2:
            ...
            if remaining > 0:
                st.warning(f"⏳ 남은 시간: {mins}분 {secs}초 (시간 종료 시 자동 불합격 처리됩니다.)")
            else:
                st.error("🚨 제한 시간이 종료되었습니다. 다시 시도해 주세요.")
                if st.button("시험 재시작"):
                    st.session_state.exam_start_time = time.time()
                    st.rerun()
                st.stop()
            ...
            st.divider()
            st.markdown("#### [ EXAM ] [필수] 사령부 정예 요원 자격 시험 (10문항 랜덤)")
            st.info("문제 은행에서 무작위로 추출된 10문제 중 8문제 이상 맞혀야 승인이 완료됩니다.")

            # --- [ SYSTEM ] 무작위 문제 은행 정의 ---
            EXAM_BANK = [
                {"q": "Q1. 우리나라 주식 차트에서 '빨간색 양초(캔들)' 그림은 무슨 뜻일까요?", "type": "radio", "opts": ["올랐다 (상승)", "떨어졌다 (하락)", "변동 없다", "거래 중지"], "a": "올랐다 (상승)"},
                {"q": "Q2. 파란색 양초(캔들) 그림은 무슨 뜻일까요?", "type": "radio", "opts": ["상승 국면", "하락 국면", "보합 국면", "매수 신호"], "a": "하락 국면"},
                {"q": "Q3. 하루 동안 사람들이 주식을 얼마나 많이 사고팔았는지 알려주는 지표는?", "type": "radio", "opts": ["거래량", "배당금", "회전율", "수익률"], "a": "거래량"},
                {"q": "Q4. 양초 모양을 닮은 막대기들로 그린 차트의 이름은 무엇일까요?", "type": "radio", "opts": ["막대 차트", "캔들 차트", "라인 차트", "그림 차트"], "a": "캔들 차트"},
                {"q": "Q5. 주식 명언 중 가장 잘 알려진 원칙은?", "type": "radio", "opts": ["발바닥에서 사서 정수리에서 팔아라", "무릎에서 사서 어깨에서 팔아라", "언제든 사고 언제든 팔아라", "사두면 언젠가 오른다"], "a": "무릎에서 사서 어깨에서 팔아라"},
                {"q": "Q6. 주식 가격의 평균을 내서 선으로 이은 그림을 무엇이라고 할까요?", "type": "radio", "opts": ["이동평균선(이평선)", "추세선", "지지선", "저항선"], "a": "이동평균선(이평선)"},
                {"q": "Q7. 단기 이평선이 장기 이평선을 아래에서 위로 뚫고 올라가는 매수 신호는?", "type": "radio", "opts": ["골든 크로스", "데드 크로스", "실버 크로스", "캔들 크로스"], "a": "골든 크로스"},
                {"q": "Q8. 주가 환경이 좋지 않을 때 매매를 쉬며 기회를 기다리는 것을 무엇이라 할까요?", "type": "radio", "opts": ["관망", "포기", "퇴장", "몰빵"], "a": "관망"},
                {"q": "Q9. 주가가 전고점을 뚫고 강력하게 상승하는 초기 시점은?", "type": "radio", "opts": ["돌파", "이탈", "보합", "조정"], "a": "돌파"},
                {"q": "Q10. 손실을 최소화하기 위해 정해진 가격에서 파는 생명줄은?", "type": "radio", "opts": ["손절", "익절", "물타기", "추격매수"], "a": "손절"},
                {"q": "Q11. 전일 종가보다 큰 차이로 높게 시작하는 것을 무엇이라 하나요? (영문 2단어)", "type": "text", "hint": "G_ _  U_ (수급의 증거)", "a": "gap up"},
                {"q": "Q12. 주식 시장의 4단계 중 가장 수익이 많이 나는 단계는? (숫자만)", "type": "text", "hint": "가장 강력한 추세 번호", "a": "2"},
                {"q": "Q13. 주가가 너무 과하게 올랐을 때 나타나는 기술적 지표 상태는?", "type": "radio", "opts": ["RSI 70 이상", "RSI 30 이하", "RSI 50", "이평선 수렴"], "a": "RSI 70 이상"},
                {"q": "Q14. 수익이 난 후 매수가격으로 손절가를 올리는 행위는? (본ㅁ)", "type": "text", "hint": "생존을 위한 필수 전술 (본_)", "a": "본절"},
                {"q": "Q15. 주가가 이미 3일 이상 오른 종목을 뒤늦게 따라 사는 위험한 행위는?", "type": "radio", "opts": ["추격 매수", "분할 매수", "예약 매수", "장외 매수"], "a": "추격 매수"},
                {"q": "Q16. 주식의 4단계 중 '매집'이 일어나는 단계는?", "type": "radio", "opts": ["1단계", "2단계", "3단계", "4단계"], "a": "1단계"},
                {"q": "Q17. 주식의 4단계 중 '분산(폭락 전)'이 일어나는 단계는?", "type": "radio", "opts": ["1단계", "2단계", "3단계", "4단계"], "a": "3단계"},
                {"q": "Q18. 마크 미너비니의 VCP 패턴에서 C가 의미하는 것은?", "type": "radio", "opts": ["Contraction (수축)", "Correction (조정)", "Chart (차트)", "Cash (현금)"], "a": "Contraction (수축)"},
                {"q": "Q19. 거래대금이 평소보다 압도적으로 터지는 시점을 무엇이라 부르나요?", "type": "radio", "opts": ["거래량 폭발", "거래 정지", "거래 마름", "거래 둔화"], "a": "거래량 폭발"},
                {"q": "Q20. 안티그래비티 사령부의 모토인 '중력'은 무엇을 의미할까요?", "type": "radio", "opts": ["공포와 탐욕의 본능", "물리학적 중력", "상승의 힘", "하락의 힘"], "a": "공포와 탐욕의 본능"},
                {"q": "Q21. 주도주가 되기 위한 필수 지표인 RS는 무엇의 약자인가요?", "type": "radio", "opts": ["Relative Strength", "Real Stock", "Red Signal", "Rising Star"], "a": "Relative Strength"},
                {"q": "Q22. VCP 패턴에서 변동성이 가장 줄어든 마지막 지점을 무엇이라 하나요?", "type": "radio", "opts": ["Pivot (피벗)", "Peak (피크)", "Pit (피트)", "Point (포인트)"], "a": "Pivot (피벗)"},
                {"q": "Q23. 주가가 볼린저 밴드 상단을 강하게 뚫고 나가는 현상은?", "type": "radio", "opts": ["밴드 돌파", "밴드 수렴", "밴드 이탈", "밴드 하회"], "a": "밴드 돌파"},
                {"q": "Q24. 계좌를 지키기 위해 수익금의 일부를 현금화하는 전술은?", "type": "radio", "opts": ["수익 실현", "물타기", "추가 매수", "손절"], "a": "수익 실현"},
                {"q": "Q25. '무중력(AntiGravity)' 상태로 진입하기 위해 가장 중요한 것은?", "type": "radio", "opts": ["심리적 평단가 우위", "엄청난 자본금", "최신 컴퓨터", "유료 리딩방"], "a": "심리적 평단가 우위"},
                {"q": "Q26. 다음 중 본데(Bonde)의 철학에 가장 가까운 것은?", "type": "radio", "opts": ["오직 강한 주도주에만 집중한다", "동전주를 사서 대박을 노린다", "남의 말만 믿고 투자한다", "공포에 전재산을 건다"], "a": "오직 강한 주도주에만 집중한다"},
                {"q": "Q27. 주식 시장에서 '뇌동매매'란 무엇일까요?", "type": "radio", "opts": ["분석 없이 충동적으로 하는 매매", "AI가 대신 해주는 매매", "원칙에 따른 기계적 매매", "장기 투자"], "a": "분석 없이 충동적으로 하는 매매"},
                {"q": "Q28. 손절을 결정할 때 가장 중요한 기준은?", "type": "radio", "opts": ["사전에 정한 퍼센트나 가격 이탈", "내일 오를 것 같은 기분", "커뮤니티의 여론", "지인의 조언"], "a": "사전에 정한 퍼센트나 가격 이탈"},
                {"q": "Q29. 쿨라매기(Kulmagi) 전략의 핵심 키워드는?", "type": "radio", "opts": ["에피소딕 피벗(EP)", "가치 투자", "배당주 투자", "역발상 투자"], "a": "에피소딕 피벗(EP)"},
                {"q": "Q30. 당신은 사령부의 명령과 원칙을 철저히 준수하겠습니까?", "type": "radio", "opts": ["예 (준수하겠습니다)", "아니오", "생각해 보겠다", "모르겠다"], "a": "예 (준수하겠습니다)"},
            ]

            # 세션 스테이트를 이용한 문제 고정
            if "selected_exam" not in st.session_state:
                st.session_state.selected_exam = random.sample(EXAM_BANK, 10)

            user_answers = []
            for i, item in enumerate(st.session_state.selected_exam):
                if item["type"] == "radio":
                    ans = st.radio(item["q"], item["opts"], index=None, key=f"q_{i}")
                else:
                    ans = st.text_input(item["q"], placeholder=item.get("hint", ""), key=f"q_{i}")
                user_answers.append(ans)

            if st.button("[ SUBMIT ] 정예 요원 입성 신청 및 시험 제출"):
                score = 0
                for ua, item in zip(user_answers, st.session_state.selected_exam):
                    if ua:
                        final_ua = str(ua).strip().lower() if item["type"] == "text" else ua
                        final_a = str(item["a"]).strip().lower() if item["type"] == "text" else item["a"]
                        if final_ua == final_a:
                            score += 1
                ...
                if score >= 8:
                    ...
                    if new_id in users:
                        ...
                        st.success(
                            f"✅ 축하합니다! {score}/10점으로 합격하셨습니다. 사령부의 지혜를 계승할 자격을 증명하셨습니다. 로그인을 진행해 주십시오."
                        )
                        st.balloons()
                        # 시험 데이터 초기화
                        if "selected_exam" in st.session_state:
                            del st.session_state.selected_exam
                        time.sleep(2)
                        st.rerun()
                else:
                    st.error(
                        f"❌ {score}/10점. 사령부의 철학을 더 공부하고 와주시기 바랍니다. (8점 이상 합격)"
                    )
                    if st.button("🔄 다른 문제로 다시 도전"):
                        if "selected_exam" in st.session_state:
                            del st.session_state.selected_exam
                        st.rerun()
    st.stop()
...
if page.startswith("3-a."):
    ...
    with st.spinner("사령부 통합 리더보드 분석 중..."):
        all_tickers = list(TICKER_NAME_MAP.keys())
        df_rank = get_leaderboard_data_v9(all_tickers)
        ...
        else: st.info("랭킹 데이터를 산출할 수 없습니다.")
...
<b style='color: #00FF00;'>[ TARGET ]:</b> {item["TP"]}
    ...
    st.markdown(
        "<div class='glass-card'>사령부 마번 시트의 데이터를 분석하여 현재 가장 뜨거운 산업군을 실시간 추출합니다.</div>",
        unsafe_allow_html=True,
    )
    
    # [ SECURE ] 사용자 제공 마번 시트 URL (gid=1499398020)
    ...
    @st.cache_data(ttl=3600)
    def analyze_industry_trends(url):
        try:
            df_raw = pd.read_csv(url)
            # 최근 5일(열) 데이터 합산
            all_tickers = []
            for col in df_raw.columns[:5]:
                all_tickers.extend(df_raw[col].dropna().astype(str).str.strip().str.upper().tolist())
            
            from collections import Counter
            counts = Counter(all_tickers)
            top_tickers = [t for t, _ in counts.most_common(20)]
            
            # 티커별 산업군 매핑 (yf 호출 최소화 위해 캐싱 및 제한적 수행)
            industry_map = {}
            for t in top_tickers:
                try:
                    # 한국 주식은 티커명으로 유추하거나 info 호출
                    tk = yf.Ticker(t)
                    info = tk.info
                    ind = info.get("sector") or info.get("industry") or "기타/분류중"
                    industry_map[t] = ind
                except:
                    industry_map[t] = "기타/분류중"
            
            # 산업군별 빈도 계산
            industry_counts = Counter([industry_map.get(t, "기타/분류중") for t in all_tickers if t in industry_map])
            return pd.DataFrame(industry_counts.most_common(10), columns=["산업군(Sector)", "강도(Score)"])
        except:
            return pd.DataFrame()

    with st.spinner("WAIT: 마번 시트 데이터 기반 산업군 정밀 분석 중..."):
        try:
            df_ind = analyze_industry_trends(SHEET_URL)
            if not df_ind.empty:
                # 시각화
                fig = px.bar(df_ind, x="강도(Score)", y="산업군(Sector)", orientation='h',
                             color="강도(Score)", color_continuous_scale="Viridis",
                             template="plotly_dark")
                fig.update_layout(height=400, margin=dict(l=20, r=20, t=20, b=20))
                st.plotly_chart(fig, use_container_width=True)
                
                st.dataframe(df_ind, use_container_width=True, hide_index=True)
                st.success(f"[ SUCCESS ] 산업군 분석 완료! (갱신일: {datetime.now().strftime('%Y-%m-%d')})")
            else:
                st.info("현재 분석할 수 있는 시트 데이터가 없거나 형식이 다릅니다.")
        except Exception as e:
            st.error(f"[ ERROR ] 산업동향 분석 실패: {e}")
...
<b style='color:#FFD700;'>모두가 경제적 자유를 얻었으면 좋겠습니다.</b> 정규직, 계약직 차별 없는 사회를 만들고 싶습니다.
    ...
    st.markdown("<div class='glass-card'>오늘의 한 줄 다짐과 함께 사령부 출석을 기록합니다.</div>", unsafe_allow_html=True)
    
    with st.form("attendance_form", clear_on_submit=True):
        comment = st.text_input("오늘의 한 줄 다짐", placeholder="예: 오늘도 원칙 매매! 뇌동 매매 금지.")
        if st.form_submit_button("출석체크 완료"):
            if comment:
                now_kst = datetime.now(pytz.timezone("Asia/Seoul")).strftime("%Y-%m-%d %H:%M:%S")
                u_id = st.session_state.current_user
                u_grade = users.get(u_id, {}).get("grade", "회원")
                
                new_att = pd.DataFrame([[now_kst, u_id, comment, u_grade]], columns=["시간", "아이디", "인사", "등급"])
                safe_write_csv(new_att, ATTENDANCE_FILE, mode="a", header=not os.path.exists(ATTENDANCE_FILE))
                st.success(f"✅ {u_id} 요원님, 오늘자 출석이 완료되었습니다!")
                st.balloons()
            else:
                st.warning("한 줄 다짐을 입력해주세요.")
...
# --- [ MARKET ANALYSIS ] ---
    ...
    st.header("[ TREND ] 마켓 트렌드 요약 (Real-time)")
    st.markdown("<div class='glass-card'>글로벌 주요 지수의 실시간 흐름을 사령부 기준(Red=Up, Blue=Down)으로 분석합니다.</div>", unsafe_allow_html=True)
    ...
    with st.spinner("지수 데이터 분석 중..."):
        ...
        try:
            # 일괄 다운로드
            idx_data = yf.download(list(indices.values()), period="2d", progress=False)
            close_idx = idx_data["Close"]
            
            cols = st.columns(3)
            for i, (name, ticker) in enumerate(indices.items()):
                if ticker in close_idx.columns:
                    hist = close_idx[ticker].dropna()
                    if len(hist) >= 2:
                        curr = hist.iloc[-1]
                        prev = hist.iloc[-2]
                        diff = curr - prev
                        ...
                        # [ ACTION ] 한국식 색상 통일: 상승 빨강, 하락 파랑
                        color = "#FF4B4B" if diff >= 0 else "#4B4BFF"
                        mark = "▲" if diff >= 0 else "▼"
                        
                        with cols[i % 3]:
                            st.markdown(f"""
                            <div class='glass-card' style='padding: 15px; border-top: 3px solid {color}; text-align: center;'>
                                <div style='color: #888; font-size: 0.8rem; font-weight: bold;'>{name}</div>
                                <div style='font-size: 1.5rem; font-weight: 900; margin: 10px 0;'>{curr:,.2f}</div>
                                <div style='color: {color}; font-weight: bold; font-size: 1rem;'>
                                    {mark} {abs(diff):,.2f} ({pct:+.2f}%)
                                </div>
                            </div>
                            """, unsafe_allow_html=True)
        except Exception as e:
