        ...
        if os.path.exists(CHAT_FILE):
            ...
            for idx, row in display_df.iterrows():
                ...
                if st.session_state.get("editing_msg_idx") == idx:
                    ...
                    # 반응 및 관리 버튼 섹션
                    react_col1, react_col2, react_col3, react_col4 = st.columns(
                        [7, 1, 1, 1]
                    )
                    ...
                    with react_col2:
                        likes_count = (
                            int(row.get("좋아요", 0))
                            if "좋아요" in row and str(row["좋아요"]).isdigit()
                            else 0
                        )
                        if st.button(
                            f"👍 {likes_count}", key=f"like_btn_{idx}", help="공감하기"
                        ):
                            local_chat.at[idx, "좋아요"] = likes_count + 1
                            local_chat.to_csv(
                                CHAT_FILE, index=False, encoding="utf-8-sig"
                            )
...
<b style='color:#FFD700;'>모두가 경제적 자유를 얻었으면 좋겠습니다.</b> 정규직, 계약직 차별 없는 사회를 만들고 싶습니다.
    ...
    st.info(
        f"[ ADVICE ] 현재 시장 변동성 지수(VIX): **{curr_vix:.1f}** | 수치가 낮을수록 시장은 안정적(탐욕), 높을수록 불안정(공포) 상태입니다."
    )

    st.info(
        "[ ADVICE ] 위 지표를 통해 대중의 광기(Greed)가 극에 달했을 때는 현금 비중을 높이고, 공포(Fear)가 극에 달했을 때는 기회를 포착하십시오."
    )
